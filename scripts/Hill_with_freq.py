...
DEFAULT_CSV = r"E:\bilat_asymmetry_analysis\data\TeLC_Silencing\WA023\20260328\right\right_ang_during.csv"
BP_LO, BP_HI = 8.0, 30.0      # whisking band (Hz) -- Nikbakht/oscillator paper (8-30 Hz)
BP_ORDER = 3                  # three-pole Butterworth, run forward+backward (zero-phase)
PHASE_THRESH = 3.0            # rad; protraction/retraction onset when |phase| exceeds this
...
def load_angle(csv_path: Path, col: str = None):
    df = pd.read_csv(csv_path)
    tcol = "Time" if "Time" in df.columns else df.columns[0]
    # choose the angle column: requested col if present, else Baseline_Subtracted,
    # else Data, else the first non-time column.
    if col and col in df.columns:
        acol = col
    elif "Baseline_Subtracted" in df.columns:
...
def decompose(ang, fps):
    ...
    b, a = butter(BP_ORDER, [BP_LO / nyq, BP_HI / nyq], btype="band")
    ...
    # cycle boundaries at phase resets of the Hilbert transform (pi -> -pi wraps).
    # The paper flags protraction/retraction onset when |phase| exceeds ~3 rad, i.e.
    # near the +/-pi wrap; detecting the reset (large negative jump in phase) marks
    # the start of each new whisk cycle.
    dphi = np.diff(phase)
    wraps = np.where(dphi < -PHASE_THRESH)[0] + 1   # phase reset: drop from ~+pi to ~-pi
    # Cycles are bracketed by resets. The leading segment (start -> first reset) and
    # trailing segment (last reset -> end) are partial cycles; keep an edge segment
    # only if it is at least half a typical cycle long (measured from the data, not
    # hardcoded) so real edge whisks count but tiny slivers do not.
    edges = np.r_[0, wraps, len(ang)]
    if len(wraps) >= 2:
        interior_len = np.median(np.diff(wraps))
    elif len(wraps) == 1:
        interior_len = max(wraps[0], len(ang) - wraps[0])
    else:
        interior_len = len(ang)
    min_len = 0.5 * interior_len
    amp_c, mid_c, t_c, bounds_kept = [], [], [], []
    for i in range(len(edges) - 1):
        s, e = edges[i], edges[i + 1]
        is_edge = (i == 0) or (i == len(edges) - 2)
        if (e - s) < (min_len if is_edge else 3):
            continue
        seg = ang[s:e]
        ...
        t_c.append((s + e) / 2.0)
        bounds_kept.append((s, e))
    t_c = np.asarray(t_c, float)
    amp_c = np.asarray(amp_c, float)
    mid_c = np.asarray(mid_c, float)

    # --- whisking frequency (the two published methods) ---
    # HILL 2011: instantaneous frequency = rate of phase advance,
    #            f(t) = d(unwrapped phase)/dt / 2pi. One value at every timepoint.
    phase_unwrapped = np.unwrap(phase)
    inst_freq = np.diff(phase_unwrapped) / (2 * np.pi) * fps
    inst_freq = np.r_[inst_freq, inst_freq[-1]] if len(inst_freq) else np.zeros_like(phase)

    # OSCILLATOR PAPER: whisking frequency = peak of the power spectrum of the
    #            band-passed angle. One characteristic frequency for the trace.
    nper = min(512, len(bp))
    if nper >= 16:
        fxx, pxx = welch(bp, fs=fps, nperseg=nper)
        band = (fxx >= BP_LO) & (fxx <= BP_HI)
        peak_freq = float(fxx[band][np.argmax(pxx[band])]) if band.any() else np.nan
    else:
        fxx, pxx, peak_freq = np.array([]), np.array([]), np.nan

    # per-cycle frequency (Hill): mean instantaneous frequency within each cycle.
    # (bounds_kept holds the (start,end) sample index of each accepted whisk cycle)
    freq_cycle = np.array([np.nanmean(inst_freq[s:e]) for (s, e) in bounds_kept]) \
                 if bounds_kept else np.array([])
    ...
    return dict(bandpassed=bp, phase=phase, amplitude=amplitude, midpoint=midpoint,
                ...
                inst_freq=inst_freq, freq_cycle=freq_cycle, t_cycle_samp=np.asarray([ (s+e)/2.0 for (s,e) in bounds_kept ], float),
                welch_f=fxx, welch_p=pxx, peak_freq=peak_freq)
...
def main():
    ...
    ap.add_argument("--col", default="Baseline_Subtracted",
                    help="Angle column to use (default Baseline_Subtracted; "
                         "use --col Data for the raw angle / true midpoint).")
    ...
    frame, ang, acol = load_angle(csv_path, args.col)
    fps = args.fps
    ...
    finite = dec['inst_freq'][np.isfinite(dec['inst_freq'])]
    if finite.size:
        print(f"  whisking frequency (Hill, instantaneous median): {np.median(finite):.1f} Hz")
    if not np.isnan(dec['peak_freq']):
        print(f"  whisking frequency (oscillator, power-spectrum peak): {dec['peak_freq']:.1f} Hz")
    if len(dec['freq_cycle']):
        per = ", ".join(f"{f:.1f}" for f in dec['freq_cycle'])
        print(f"  per-cycle frequency (Hz): {per}")
    ...
    pd.DataFrame({
        ...
        "inst_frequency_hz": dec["inst_freq"],
    }).to_csv(out_csv, index=False)
    print(f"Saved: {out_csv}")

    # figure
    t_ms = (frame - frame[0]) / fps * 1000.0
    fig, ax = plt.subplots(5, 1, figsize=(11, 10.5), sharex=True,
    ...
    ax[3].axhspan(BP_LO, BP_HI, color="0.9", alpha=0.4, zorder=0)
    ax[3].plot(t_ms, dec["inst_freq"], color="0.6", lw=1, label="Hill (instantaneous)")
    if len(dec["freq_cycle"]):
        tc_ms = (frame[dec["t_cycle_samp"].astype(int)] - frame[0]) / fps * 1000.0
        ax[3].plot(tc_ms, dec["freq_cycle"], "o-", color="#ff7f0e", lw=1.5, ms=5,
                   label="per-cycle (Hill mean)")
    if not np.isnan(dec["peak_freq"]):
        ax[3].axhline(dec["peak_freq"], color="#1f77b4", lw=1.5, ls="--",
                      label=f"oscillator (spectral peak {dec['peak_freq']:.1f} Hz)")
    ax[3].set_ylabel("Frequency (Hz)"); ax[3].legend(fontsize=8, loc="upper right")
    ax[3].set_ylim(0, 35)

    ax[4].plot(t_ms, dec["phase"], color="#9467bd", lw=1)
    ax[4].set_ylabel("Phase (rad)")
    ax[4].set_yticks([-np.pi, 0, np.pi]); ax[4].set_yticklabels(["-\u03c0", "0", "\u03c0"])
